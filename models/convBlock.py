...
class ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        print(f"Initializing ConvBlock with dropout={self.dropout}, in_channels={in_channels}, out_channels={out_channels}")
    ...
    def add_layer(self):
        ...
        if self.count < 10:  # BEST 5
            ...
            self.count += 1
# class ConvBlock(nn.Module):
#     def __init__(self, in_channels: int, out_channels: int, kernel_size: int, pooling_amount: int, dropout: float) -> None:
#         super().__init__()
#         self.count = 0
#         self.out_channels = out_channels
#         self.kernel_size = kernel_size
...
#     def add_layer(self):
#         if self.count < 10:
#             new_layer = IdentityConvLayer(channels=self.out_channels).to(self.device)
#             self.convs.insert(len(self.convs) - 1, new_layer)
#             self.count += 1
